...
var checkAddedLargeFilesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Printf("check-added-large-files called maxMib=%f\n", maxMib)
		if maxMib == 0 {
			os.Exit(0)
		}
...
func init() {
	...
	checkAddedLargeFilesCmd.Flags().BoolVar(&hint, "hint", true, "Whether to display hint")
...
func findLargeAddedFiles() int {
	...
	for _, r := range refusedFiles {
		...
		fmt.Printf("  %s: %sMiB; the maximum size allowed is %sMiB\n", r.filename, mib, _maxMib)
	}
	if hint {
